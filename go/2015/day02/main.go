...
func getShortestPerimeter(dimensions []int) int {
	a := (dimensions[0] + dimensions[1]) * 2
	b := (dimensions[1] + dimensions[2]) * 2
	c := (dimensions[0] + dimensions[2]) * 2
...
func partTwo() {
	...
	for scanner.Scan() {
		...
		dimensions := getDimensions(parts)
		ribbon := getShortestPerimeter(dimensions)
		bow := getVolume(dimensions)
		fmt.Println("dimensions:", line, "ribbon length:", ribbon+bow)
		total = total + ribbon + bow
	}
	fmt.Println("total ribbon needed:", total)
